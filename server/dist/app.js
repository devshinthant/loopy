...
const createWorker_1 = __importDefault(require("./worker/createWorker"));
const config_1 = __importDefault(require("./config"));
const createTransport_1 = __importDefault(require("./transports/createTransport"));
...
peers.on("connection", async (socket) => {
    ...
    socket.on("leave-room", ({ roomId, userId }) => {
        ...
        (0, cleanUp_1.default)({ socket, roomId });
    });
    socket.on("disconnect", () => {
        ...
        (0, cleanUp_1.default)({ socket });
        console.log("Client disconnected", socket.id);
    });
    socket.on("end-room", ({ roomId }) => {
        (0, cleanUp_1.default)({ socket, roomId, endRoom: true });
    });
    socket.on("createRoom", async ({ roomId, password, userData }, callback) => {
        ...
        if (!worker) {
            worker = await (0, createWorker_1.default)();
        }
        const router = await worker.createRouter({
            mediaCodecs: config_1.default,
        });
        const newRoom = new room_1.default(roomId, password, router);
    ...
    socket.on("createTransport", async ({ sender, roomId }, callback) => {
        ...
        if (!peer)
            return;
        const transport = await (0, createTransport_1.default)(roomId, callback);
        if (!transport) {
            return callback({
    ...
    socket.on("transport-produce", async ({ kind, rtpParameters, roomId, appData }, callback) => {
    ...
    socket.on("connectConsumerTransport", async ({ dtlsParameters, roomId }, callback) => {
        try {
            const room = exports.rooms.get(roomId);
            if (!room) {
                return callback({
    ...
    socket.on("consumeProducer", async ({ rtpCapabilities, roomId, producerId, kind }, callback) => {
        try {
            if (!producerId) {
                return callback({
    ...
    socket.on("giveMeOthers", ({ roomId }, callback) => {
        ...
        const peers = room.getPeers();
        if (!peers)
            return callback({ message: "No peers found" });
    ...
    socket.on("send-message", ({ roomId, message }, callback) => {
        ...
        if (!peer)
            return;
        const messageId = (0, uuidv4_1.uuid)();
        const messageData = {
            id: messageId,
            userData: peer.data,
            message,
